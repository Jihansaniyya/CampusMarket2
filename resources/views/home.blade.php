...
>
    <div class="relative rounded-3xl overflow-hidden shadow-xl" aria-label="Hero carousel">
        <template x-for="(banner, index) in banners" :key="index">
            <div x-show="current === index" x-transition class="relative h-72 md:h-96">
                <img :src="banner.image_url" :alt="banner.title" class="w-full h-full object-cover" loading="lazy">
                <div class="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent"></div>
                <div class="absolute inset-y-0 left-0 flex flex-col justify-center px-8 md:px-12 text-white space-y-3 max-w-lg">
                    ...
                    <a :href="banner.cta_link" class="inline-flex items-center bg-blue-600 text-white px-5 py-2.5 rounded-full w-max font-semibold hover:bg-blue-500 transition" x-text="banner.cta_text"></a>
                </div>
            </div>
        </template>

        <div class="absolute inset-y-0 left-0 right-0 flex items-center justify-between px-4">
            <button @click="go((current - 1 + banners.length) % banners.length)" class="bg-white/70 hover:bg-white rounded-full p-2" aria-label="Banner sebelumnya">
                <svg class="w-5 h-5 text-gray-800" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m15 19-7-7 7-7" /></svg>
            </button>
            <button @click="go((current + 1) % banners.length)" class="bg-white/70 hover:bg-white rounded-full p-2" aria-label="Banner selanjutnya">
                <svg class="w-5 h-5 text-gray-800" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m9 5 7 7-7 7" /></svg>
            </button>
        </div>
        ...
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            <template x-for="(_, index) in banners" :key="'indicator-' + index">
                <button
                    ...
                    :aria-label="'Pergi ke banner ' + (index + 1)"
                ></button>
            </template>
        </div>
    </div>
    ...
    <section class="mt-12 bg-gradient-to-r from-blue-50 via-white to-orange-50 rounded-3xl p-6 md:p-10 flex flex-col md:flex-row items-center gap-6">
        <div>
            ...
            <p class="text-gray-600 mt-2">Gunakan kode <span class="font-semibold text-blue-600">CAMPUSNEW</span> sebelum 30 November.</p>
        </div>
        <a href="#" class="inline-flex items-center bg-blue-600 text-white px-6 py-3 rounded-full font-semibold hover:bg-blue-500 transition">Klaim Sekarang</a>
    </section>
