...
<article class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition flex flex-col">
    <div class="relative">
        <img src="{{ $product['image_url'] }}" alt="{{ $product['name'] }}" loading="lazy" class="w-full h-56 object-cover rounded-t-2xl">
        @if (!empty($product['badge']))
            <span class="absolute top-4 left-4 bg-orange-400 text-white text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wide">{{ $product['badge'] }}</span>
        @endif
        <button
            type="button"
            ...
            @click="$store.quickView.openModal({{ $productJson }})"
        ...
        <div class="mt-auto">
            <div class="flex items-baseline gap-2">
                @if ($product['sale_price'])
                    ...
                    <span class="text-xl font-semibold text-blue-600">Rp {{ number_format($product['sale_price'], 0, ',', '.') }}</span>
                @else
                    <span class="text-xl font-semibold text-blue-600">Rp {{ number_format($product['price'], 0, ',', '.') }}</span>
                @endif
            </div>
            <div class="flex gap-3 mt-4">
                <button class="flex-1 bg-blue-600 text-white rounded-full py-2.5 text-sm font-semibold hover:bg-blue-500 transition">Tambah ke Keranjang</button>
                <a href="{{ url('product/' . $product['slug']) }}" class="px-4 py-2.5 border border-gray-200 rounded-full text-sm font-medium text-gray-700 hover:border-blue-500 hover:text-blue-600 transition">Detail</a>
            </div>
        </div>
    </div>
</article>