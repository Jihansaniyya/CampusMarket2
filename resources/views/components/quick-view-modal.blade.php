<div x-cloak x-show="$store.quickView.open" x-transition.opacity
    class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4" role="dialog" aria-modal="true"
    aria-labelledby="quick-view-title">
    <div id="quick-view-modal" tabindex="-1"
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-100">
            ...
            <button @click="$store.quickView.closeModal()" class="text-gray-400 hover:text-gray-600"
        ...
        <div class="grid md:grid-cols-2">
            <div class="p-6 bg-gray-50">
                <img :src="$store.quickView.product.image_url" :alt="$store.quickView.product.name"
                    class="rounded-2xl w-full h-80 object-cover bg-gray-100" loading="lazy">
            </div>
            <div class="p-6 flex flex-col gap-4">
                ...
                <div class="flex items-center gap-2 text-amber-500">
                    ...
                    <span class="text-gray-400"
                        x-text="'(' + ($store.quickView.product.reviews_count ?? 0) + ' reviews)'"></span>
                </div>
                <div class="flex items-baseline gap-2">
                    <template x-if="$store.quickView.product.sale_price">
                        <span class="text-sm text-gray-400 line-through"
                            x-text="'Rp ' + Number($store.quickView.product.price).toLocaleString('id-ID')"></span>
                    </template>
